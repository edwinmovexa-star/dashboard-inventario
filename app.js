...
function fmtDate(iso){ if(!iso)return "-"; return new Intl.DateTimeFormat("es-MX",{day:"2-digit",month:"short",year:"numeric"}).format(new Date(iso+"T12:00:00")); }
...
function setOptions(id, arr){ $(id).innerHTML = arr.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join(""); }
function statusBadge(status){
  const map={"Pendiente":"b-pending","Analizando":"b-analysis","En proceso":"b-process","En revisión":"b-review","Finalizado":"b-done","Bloqueado":"b-blocked",
  "Detectada":"b-pending","En corrección":"b-process","Resuelta":"b-done"};
  return `<span class="badge ${map[status]||"b-analysis"}">${esc(status)}</span>`;
}
function priorityClass(p){ return "priority-"+({Urgente:"urgent",Alta:"high",Media:"medium",Baja:"low"}[p]||"medium"); }
...
function renderDashboard(){
  ...
  let records=operations.filter(r=>{
    ...
    if(period==="week"){ const start=new Date(now); start.setDate(now.getDate()-((now.getDay()+6)%7)); const end=new Date(start); end.setDate(start.getDate()+6); return d>=start&&d<=end; }
  ...
  const counts=STATUSES.map(s=>({s,c:activities.filter(a=>a.status===s).length})).filter(x=>x.c);
...
function renderActivities(){
  ...
  const st=$("filterStatus").value, pr=$("filterProcess").value, pi=$("filterPriority").value;
  ...
  $("activitiesTable").innerHTML=list.length?list.map(a=>`<tr>
    ...
    <td>${fmtDate(a.date)}</td><td><div class="progress"><span style="width:${Number(a.progress)||0}%"></span></div><small>${Number(a.progress)||0}%</small></td>
    <td><div class="row-actions"><button class="mini-btn" onclick="editActivity('${a.id}')">✏️</button><button class="mini-btn" onclick="deleteActivity('${a.id}')">🗑️</button></div></td>
  </tr>`).join(""):`<tr><td colspan="7">No hay actividades.</td></tr>`;
}

function renderOperation(){
  $("operationTable").innerHTML=operations.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(r=>`<tr><td>${fmtDate(r.date)}</td><td>${r.products}</td><td>${r.boxes}</td><td>${r.sku}</td><td>${r.diff}</td><td>${r.fixed}</td><td>${r.completion}%</td></tr>`).join("") || `<tr><td colspan="7">Aún no hay registros.</td></tr>`;
}
...
function dateISOFromDate(d){ return new Date(d.getFullYear(),d.getMonth(),d.getDate(),12).toISOString().slice(0,10); }
function renderPlanner(){
  const content=$("plannerContent");
  if(plannerMode==="day"){
    const iso=dateISOFromDate(plannerDate);
    $("plannerTitle").textContent=new Intl.DateTimeFormat("es-MX",{weekday:"long",day:"numeric",month:"long"}).format(plannerDate);
    ...
    const d=new Date(plannerDate); const monday=new Date(d); monday.setDate(d.getDate()-((d.getDay()+6)%7));
    ...
    $("plannerTitle").textContent=new Intl.DateTimeFormat("es-MX",{month:"long",year:"numeric"}).format(plannerDate);
    const first=new Date(y,m,1), startOffset=(first.getDay()+6)%7, start=new Date(y,m,1-startOffset);
    const heads=["Lun","Mar","Mié","Jue","Vie","Sáb","Dom"].map(h=>`<div class="month-head">${h}</div>`).join("");
    const cells=Array.from({length:42},(_,i)=>{const x=new Date(start);x.setDate(start.getDate()+i);const iso=dateISOFromDate(x);const list=activities.filter(a=>a.date===iso);const cls=[x.getMonth()!==m?"muted":"",iso===todayISO()?"today":""].join(" ");return `<div class="month-day ${cls}"><div class="month-num">${x.getDate()}</div>${list.slice(0,3).map(a=>`<div class="month-task">${esc(a.title)}</div>`).join("")}</div>`}).join("");
...
window.deleteActivity=async id=>{
  if(confirm("¿Eliminar esta actividad?")){
    try{ await deleteRecord("activities",id); showToast("Actividad eliminada"); }
...
window.deleteIncident=async id=>{
  if(confirm("¿Eliminar esta incidencia?")){
    try{ await deleteRecord("incidents",id); showToast("Incidencia eliminada"); }
    catch(err){ console.error(err); showToast("No se pudo eliminar"); }
  }
};
...
function renderAll(){ renderDashboard();renderActivities();renderOperation();renderIncidents();renderPlanner(); }

document.addEventListener("DOMContentLoaded",()=>{
  $("todayLabel").textContent=new Intl.DateTimeFormat("es-MX",{weekday:"long",day:"numeric",month:"long"}).format(new Date());
  $("opDate").value=todayISO(); $("incDate").value=todayISO();
  setOptions("actProcess",PROCESSES);setOptions("actPriority",PRIORITIES);setOptions("actStatus",STATUSES);setOptions("actFrequency",FREQUENCIES);
  setOptions("incType",INCIDENT_TYPES);setOptions("incPriority",PRIORITIES);setOptions("incStatus",INCIDENT_STATUSES);
  ...
  $("activityForm").onsubmit=async e=>{
    ...
    const item={title:$("actTitle").value.trim(),process:$("actProcess").value,priority:$("actPriority").value,status:$("actStatus").value,date:$("actDate").value,dueDate:$("actDueDate").value,frequency:$("actFrequency").value,progress:Number($("actProgress").value)||0,notes:$("actNotes").value.trim(),assignedTo:currentUser?.uid||"",assignedName:currentProfile?.name||currentUser?.email||"",createdBy:currentUser?.uid||""};
    try{
    ...
    }catch(err){console.error(err);showToast("No se pudo guardar la actividad");}
  };
  $("incidentForm").onsubmit=async e=>{
    e.preventDefault();
    const item={title:$("incTitle").value.trim(),type:$("incType").value,reference:$("incReference").value.trim(),priority:$("incPriority").value,status:$("incStatus").value,date:$("incDate").value,description:$("incDescription").value.trim(),createdBy:currentUser?.uid||""};
    try{
      await createRecord("incidents",item);
      e.target.reset();$("incDate").value=todayISO();$("incidentModal").classList.add("hidden");showToast("Incidencia registrada");
    }catch(err){console.error(err);showToast("No se pudo guardar la incidencia");}
  };
  $("operationForm").onsubmit=async e=>{
    e.preventDefault(); const d=$("opDate").value;
    const item={date:d,products:Number($("opProducts").value)||0,boxes:Number($("opBoxes").value)||0,sku:Number($("opSku").value)||0,diff:Number($("opDiff").value)||0,fixed:Number($("opFixed").value)||0,completion:Number($("opCompletion").value)||0,notes:$("opNotes").value.trim(),userId:currentUser?.uid||"",userName:currentProfile?.name||currentUser?.email||""};
    try{await saveDailyRecord(item);showToast("Jornada guardada");}
    catch(err){console.error(err);showToast("No se pudo guardar la jornada");}
  };
  ...
  document.querySelectorAll(".seg-btn").forEach(b=>b.onclick=()=>{document.querySelectorAll(".seg-btn").forEach(x=>x.classList.remove("active"));b.classList.add("active");plannerMode=b.dataset.planner;renderPlanner();});
  $("plannerPrev").onclick=()=>{ if(plannerMode==="day")plannerDate.setDate(plannerDate.getDate()-1); else if(plannerMode==="week")plannerDate.setDate(plannerDate.getDate()-7); else plannerDate.setMonth(plannerDate.getMonth()-1); renderPlanner();};
  $("plannerNext").onclick=()=>{ if(plannerMode==="day")plannerDate.setDate(plannerDate.getDate()+1); else if(plannerMode==="week")plannerDate.setDate(plannerDate.getDate()+7); else plannerDate.setMonth(plannerDate.getMonth()+1); renderPlanner();};
  $("plannerToday").onclick=()=>{plannerDate=new Date();renderPlanner();};
